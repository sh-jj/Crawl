...
def getNews(url):  
    ...
    if not(soup.find('div', {'id': 'main_content'})): return   
    main_content = soup.find('div', {'id': 'main_content'})   #新闻正文内容 
    cont_img = soup.find('div', {'id': 'main_content'})
    
    imgList = getImg(cont_img)
    imgList2 = getImg2(cont_img)
    imgList3 = getImg3(cont_img)
    ...
    imgCount=0
    for imgPath in imgList3:
        f=open("data/"+str(count)+"/"+str(imgCount)+".png",'wb')
        f.write((urllib.request.urlopen(imgPath)).read())
        f.close()
        imgCount+=1
...
def dfs(url):  
    global local
    pattern1 = 'http://culture\.ifeng\.com\/[a-z0-9_\/\.]*$'     #可以继续访问的url规则  
    pattern2 = 'http://culture\.ifeng\.com\/a\/[0-9]{8}\/[0-9]{8}\_0\.shtml$'  #解析新闻信息的url规则  
