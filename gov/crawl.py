...
def getNews(url):  
    ...
    if not(soup.find('div', {'id': 'yc_con_txt'})): return   
    main_content = soup.find('div', {'id': 'yc_con_txt'})   #新闻正文内容 
      
    imgList = getImg(main_content)
    imgList2 = getImg2(main_content)
    imgList3 = getImg3(main_content)
...
def dfs(url):  
    global local
    pattern1 = 'http://gov\.ifeng\.com\/[a-z0-9_\/\.]*$'     #可以继续访问的url规则  
    pattern2 = 'http://gov\.ifeng\.com\/a\/[0-9]{8}\/[0-9]{8}\_0\.shtml$'  #解析新闻信息的url规则  
